...
interface IMembershipManager {
    ...
    function wrapEthForEap(uint256 _amount, uint256 _amountForPoint, uint32  _eapDepositBlockNumber, uint256 _snapshotEthAmount, uint256 _points, bytes32[] calldata _merkleProof, bytes memory _sig) external payable returns (uint256);
    function wrapEth(bytes memory _sig, uint256 _amount, uint256 _amountForPoint, bytes32[] calldata _merkleProof) external payable returns (uint256);

    function topUpDepositWithEth(bytes memory _sig, uint256 _tokenId, uint128 _amount, uint128 _amountForPoints, bytes32[] calldata _merkleProof) external payable;
